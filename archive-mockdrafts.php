		...
		<div class="col-sm-8 main-col">
			...
			<div class="col-sm-6" style="text-align: right; padding-top: 20px;">
				...
				<?php if ( !$member || $member == 'all' || $member == 'staff' ) {
					?>
					<form id="class_form" action="<?php bloginfo('url'); ?>/mockdrafts/" method="GET">

						<select class="form-control" id="class_select" name="member" style="margin-top: 5px; width: 155px; float: right;">
							<option value="all"<?php if ($member == 'all' || $member == null) echo "selected='selected'"; ?>>Staff and Members</option>
							<option value="staff"<?php if ($member == 'staff' ) echo "selected='selected'"; ?>>Staff Only</option>
						</select>
						<span style="margin-right: 10px; margin-top: 10px; float: right; font-style: italic;">View Mocks From: </span>
					</form>
					<?php
						}
				?>
			...
			<hr>
			<?php get_template_part('partials/listing', 'mockdrafts'); ?>
